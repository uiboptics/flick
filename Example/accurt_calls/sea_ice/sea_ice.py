...
class sea_ice:
    ...
    def __init__(self, flick_radiation):
        ...
        self.fr.set('ice_presence',[1, 1,0,0])
        self.fr.set('ice_bubble_fraction',[0.01, 0.01, 0.01, 0.01])
        self.fr.set('ice_brine_fraction',[0.01, 0.01, 0.01, 0.01])
...
def plot_radiance():
    ...
    plt.grid()
    plt.xlabel('Wavelength [nm]')
    plt.ylabel(r'Nadir radiance [mW m$^{-2}$ sr$^{-1}$ nm$^{-1}$]')
